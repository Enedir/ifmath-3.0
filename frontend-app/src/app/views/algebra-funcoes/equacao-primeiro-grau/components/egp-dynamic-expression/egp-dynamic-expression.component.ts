...
export class EgpDynamicExpressionComponent {
  ...
  constructor(private fb: FormBuilder, private servico: ServicoEquacaoPrimeiroGrau) { }

  public onSubmit(): void {
    this.loading = true;
    this.results = [];

    this.servico.postDynamicResolution(this.formModel.value).pipe(
      take(1),
      tap(() => {
        this.loading = false;
      }), ).subscribe(
        (response: RespostaJson) => {
          if (response.sucesso) {
            this.results = ( response.objeto as ResultadoViewModel[]);
          } else {
